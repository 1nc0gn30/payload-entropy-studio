"""
Cross-platform compatibility utilities for payload-entropy-studio.
...
def get_default_storage_dir() -> Path:
    """Determine OS-specific default storage directory for security payload reports."""
    if IS_TERMUX:
        base = Path(os.environ.get("HOME", "/data/data/com.termux/files/home")) / ".payload_entropy"
    elif IS_WINDOWS:
        base = Path(os.environ.get("LOCALAPPDATA", Path.home() / "AppData" / "Local")) / "PayloadEntropy"
    elif IS_MACOS:
        base = Path.home() / "Library" / "Application Support" / "PayloadEntropy"
    else:
        base = Path.home() / ".local" / "share" / "payload_entropy"
